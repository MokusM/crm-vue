    ...
    <section v-else>
      <div v-for="cat in categories" :key="cat.id">
        <p >
          ...
          {{ cat.spend | currency }} из {{ cat.limit | currency('UAH') }}
        </p>
  ...
  async mounted () {
    ...
    this.categories = categories.map(cat => {
      const spend = records
        ...
        .reduce((total, record) => {
          const totalRecord = total += +record.amount
          return totalRecord
        }, 0)
      ...
      const tooltip = `${tooltipValue < 0 ? 'Превешения на' : 'Осталось'} ${currencyFilter(Math.abs(tooltipValue))}`
      return {
        ...cat,
        progressPercent,
        progressColor,
